 ...
 */
package com.example.anamuslim.prayertimecalculation;
...
public class PrayerTimes {
    ...
    public static Way getDefaultWay(String countryCode) {
        ...
        if("CM, CF, CD, CG, CI, EG, GH, IQ, KE, LY, MY, ML, SN, SO, SD, TN".contains(countryCode)) return Way.PTC_WAY_EGYPT;

        if("AF, AL, BD, IN, PK, WF".contains(countryCode)) return Way.PTC_WAY_KARACHI;

        if("CA, US".contains(countryCode)) return Way.PTC_WAY_ISNA;

        if("BH, KW, OM, QA, SA, AE, YE".contains(countryCode)) return Way.PTC_WAY_UMQURA;
